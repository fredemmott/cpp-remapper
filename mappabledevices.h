...
struct MappableOutput {
  typedef fredemmott::vjoypp::OutputDevice Device;

  explicit MappableOutput(uint8_t vjoy_id);
  MappableOutput(Device* dev);
...
struct MappableInput {
  typedef fredemmott::gameinput::InputDevice Device;
  MappableInput(Device* dev);
  Device* getDevice() const;

  AxisSource axis(uint8_t id) const;
  ButtonSource button(uint8_t id) const;
  HatSource hat(uint8_t id) const;

  // Convenience

  const size_t AxisCount;
  const size_t ButtonCount;
  const size_t HatCount;
  ...
  const HatSource Hat1, Hat2, Hat3, Hat4;
